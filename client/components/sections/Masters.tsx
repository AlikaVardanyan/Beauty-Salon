...
export default function Masters() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(loadMasters, 1000); // обновление каждую секунду
          ...
          {masters.map((master, index) => (
            ...
            >
              <div className="relative aspect-[3/4] rounded-3xl overflow-hidden mb-6 shadow-md transition-shadow group-hover:shadow-xl">
                <img 
                  src={master.photo || "https://images.unsplash.com/photo-1595152772835-219674b2a8a6?auto=format&fit=crop&q=80&w=800"} 
                  alt={master.name} 
              ...
              <p className="text-primary font-medium text-sm">{master.specialty}</p>
            </motion.div>
          ))}
        </div>
      </div>
